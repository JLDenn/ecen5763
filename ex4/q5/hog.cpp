...
static const string keys = "{ help h   |   | print help message }"
						   ...
						   "{ save     |   | save each processed frame }";
...
int main(int argc, char** argv){
	...
	string saveOut = parser.get<string>("save");
    ...
    for (;;){
        ...
        // show the window
        {
            ...
            buf << "Mode: " << detector.modeName() << " ||| "
                << "FPS: " << fixed << setprecision(1) << (getTickFrequency() / (double)(getTickCount() - t));
			t = getTickCount();
		...
		if(parser.has("save")){
			ostringstream buf;
			buf << "frame" << frameIdx++ << ".jpg";
			imwrite(buf.str(), frame);
		}
